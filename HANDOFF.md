...
LaTeX on this machine, so it has **never been compiled**. `paper/check_tex.py`
stands in: it checks environments, `\ref`/`\cite` resolution, missing figure
files and estimated length. Run it before every push.
