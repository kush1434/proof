...
`paper/proof_bibm2026.tex` — IEEEtran, ~4.8 of 5 pages estimated. There is no
LaTeX on this machine, so it has **never been compiled**. Three checkers stand
in for that compile, and the `paper` workflow runs all of them on every push:
...
|---|---|---|
| 1 | `fig1_personalisation.py` | the k-meal curve, with paired CIs |
| 2 | `fig2_r4_divergence.py` | R-4: internal monotone, reported wraps |
| 3 | `fig3_architecture.py` | where the guard taps the weight stream |
| 4 | `fig4_layout.py` | the routed tile, with a 1:1 inset |

**Still open, and only Kush can close them:**

- the author block is a placeholder — authorship is undecided;
...
- the real page count, on the first Overleaf build.
